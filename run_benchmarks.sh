#!/bin/bash
# run_benchmarks.sh — Run Godom benchmarks via Go WebAssembly target using Node.js
...
echo "==> Running benchmarks for GOOS=js GOARCH=wasm ..."
GOOS=js GOARCH=wasm go test -exec="node $WASM_EXEC" -bench=. -benchmem ./benchmarks/...
